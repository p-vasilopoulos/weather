...
export const crossplugin = {
  ...
  afterInit: function (chart: any) {
    ...
    if (syncEnabled) {
      chart.crosshair.syncEventHandler = function (this: any, e: any) {
        this.handleSyncEvent(chart, e);
      }.bind(this);

      chart.crosshair.resetZoomEventHandler = function (this: any, e: any) {
  ...
  afterDestroy: function (chart: any) {
    var syncEnabled = this.getOption(chart, 'sync', 'enabled');
    if (syncEnabled) {
  ...
  panZoom: function (chart: any, increment: any) {
  ...
  getOption: function (chart: any, category: any, name: any) {
    return chart.options.plugins.crosshair[category]
  ...
  handleSyncEvent: function (chart: any, e: any) {
    var syncGroup = this.getOption(chart, 'sync', 'group');
  ...
  afterEvent: function (chart: any, event: any) {
    ...
    if (!e.stop && syncEnabled) {
      var event = new CustomEvent('sync-event') as any;
  ...
  doZoom: function (chart: any, start: any, end: any) {
    ...
    let beforeZoomCallback = chart.options.plugins.crosshair.callbacks
      ? chart.options.plugins.crosshair.callbacks.beforeZoom
      : undefined;
    if (
      !beforeZoomCallback &&
      defaultOptions !== undefined &&
      defaultOptions.callbacks !== undefined
    ) {
      beforeZoomCallback = defaultOptions.callbacks.beforeZoom;
    }
    // notify delegate
  ...
  drawZoombox: function (chart: any) {
  ...
  drawTraceLine: function (chart: any) {
    var yScale = this.getYScale(chart);
  ...
  drawTracePoints: function (chart: any) {
    for (
      var chartIndex = 0;
  ...
  interpolateValues: function (chart: any) {
    for (
      var chartIndex = 0;
      chartIndex < chart.data.datasets.length;
      chartIndex++
      ...
      var index = data.findIndex((o: any) => {
        return o.x >= xValue;
      });
...
export function interpolate(chart: any, e: any, options: any) {
  var items = [];
  ...
  for (
    ...
    var index = data.findIndex(function (o: any) {
      return o.x >= xValue;
    });
